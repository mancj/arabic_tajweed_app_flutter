...
class ExerciseGenerator {
  ...
  List<Exercise> build({
    ...
    // Кого в этом уроке уже просили назвать вслух: раз на букву за урок.
    final spoken = <String>{};
    final seen = <Atom, int>{};
  ...
  /// спросить сразу. Не больше раза на букву за урок, иначе урок
  /// превращается в диктовку. Только в активном слоте — том, что иначе ушёл
  /// бы письму: так голос не ломает чередование с узнаванием. Шанс тот же,
  /// что у вопроса о форме: иначе каждый активный слот уходил бы на запись.
  bool _saysName(
    Atom atom,
    _Slot slot,
    Set<String> spoken, {
    ...
    if (_random.nextInt(3) != 0) return false;
    spoken.add(letterId);
    return true;
  }
