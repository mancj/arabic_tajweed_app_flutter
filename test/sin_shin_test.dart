...
void main() {
  ...
  test('син и шин делят одну основу', () {
    ...
    expect(
      StrokeSignature.ofPaths(sin.parts.first.paths)!
  ...
  test('основа син не путается с чашей ба', () {
    final sin = StrokeSignature.ofPaths(
        letter('sin_base').resolve(canvasSize, padding: padding).parts.first.paths)!;
  ...
  test('части буквы могут быть другой развесовки', () {
    ...
    bool accepts(List<Offset> pts) => matcher
        .match(
          target: target,
          strokes: [
            DrawingStroke(points: pts, color: const Color(0xFF000000), width: 20),
          ],
          penWidth: 20,
          structural: true,
        )
    ...
    expect(accepts(points.sublist(0, (points.length * .62).round())), isFalse,
        reason: 'два зубца вместо трёх');
    expect(accepts([...points.sublist(0, (points.length * .2).round()), ...points]),
        isFalse,
  ...
  testWidgets('шин собирается по памяти: основа и три точки', (tester) async {
    ...
    for (var i = 0; i < dots.length; i++) {
      ...
      await tester.pumpAndSettle();
      expect(progress.last.isComplete, i == dots.length - 1,
  ...
  testWidgets('две точки вместо трёх не завершают шин', (tester) async {
    ...
    for (var d = 2.0; d < metric.length; d += 2) {
      await gesture.moveTo(metric.getTangentForOffset(d)!.position + box.topLeft);
    }
    await gesture.up();
    await tester.pumpAndSettle();
    ...
    expect(progress.last.completed, 1, reason: 'без третьей точки это ﺱ, а не ﺵ');
  });
}
