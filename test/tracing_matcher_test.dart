...
void main() {
  ...
  test('половина буквы не засчитывается', () {
    final result = matcher.match(
      target: shape.whole,
      strokes: [strokeOf(traceOf(shape, 4, portion: 0.5))],
    );
  ...
  test('без точки под буквой не засчитывается', () {
    final result = matcher.match(
      target: shape.whole,
      strokes: [strokeOf(traceOf(shape, 6))],
    );
    expect(result.isMatch, isFalse);
  });
  ...
  test('каракули мимо не засчитываются', () {
    final points = [
      for (var i = 0; i < 120; i++) Offset(40.0 + i, 40.0 + (i % 20)),
    ];
    final result = matcher.match(
      target: shape.whole,
  ...
  test('мазня поверх буквы не засчитывается', () {
    final points = [
      ...traceOf(shape, 6),
      for (var i = 0; i < 300; i++)
        Offset(random.nextDouble() * 360, random.nextDouble() * 400),
    ];
    final result = matcher.match(
      target: shape.whole,
      strokes: [strokeOf(points)],
