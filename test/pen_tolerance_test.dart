...
void main() {
  ...
  TracingMatchResult match(List<Offset> points, {double? penWidth}) => matcher.match(
        target: base,
        strokes: [
          DrawingStroke(points: points, color: const Color(0xFF000000), width: pen),
        ],
        penWidth: penWidth,
      );
  ...
  test('линия буквы тоньше пера — допуск считается от пера', () {
    ...
    expect(match(drifted).isMatch, isFalse,
        reason: 'без учёта пера тот же штрих отвергался');
  });

  test('сдвиг больше пера не проходит', () {
    expect(match(trace(shift: const Offset(-7, -18)), penWidth: pen).isMatch, isFalse);
  });
    ...
    List<Offset> spiked(double amount) => [
          for (var d = 0.0; d < metric.length; d += 2)
            () {
              ...
              return Offset(
                  p.dx, p.dy - exp(-pow((t - .5) / .13, 2)) * height * amount);
            }(),
        ];
